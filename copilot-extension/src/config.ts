...
function getConfigDir(): string {
    if (!configDir) {
        configDir = path.join(process.env.USERPROFILE || '', '.vscode', 'extensions', 'local-copilot');
...
export function loadConfig(): AppConfig {
    ...
    try {
        ...
        console.error('[Local Copilot] Failed to parse config file:', e.message);
    }
...
export function getActiveProvider(): string {
    ...
    const fallback = vsc.get<string>('aiProvider', 'ollama');
    return cfg.aiProvider || fallback;
}

export function getSystemPrompt(): string {
    const cfg = loadConfig();
    const vsc = vscode.workspace.getConfiguration('local-copilot');
    const fallback = vsc.get<string>('systemPrompt', DEFAULT_SYSTEM_PROMPT);
    return cfg.systemPrompt || fallback;
}
...
export function getProviderConfig(type: string): ProviderConfig {
    ...
    switch (type) {
        case 'ollama':
            out.endpoint = out.endpoint || vsc.get<string>('ollamaEndpoint', 'http://127.0.0.1:11434');
            out.model = out.model || vsc.get<string>('ollamaModel', 'qwen2.5-coder:3b');
            out.host = out.host || vsc.get<string>('ollamaHost', '0.0.0.0');
            break;
        case 'lmstudio':
            out.endpoint = out.endpoint || vsc.get<string>('lmstudioEndpoint', 'http://127.0.0.1:1234/v1');
            out.model = out.model || vsc.get<string>('lmstudioModel', '');
            break;
        case 'janai':
            out.endpoint = out.endpoint || vsc.get<string>('janaiEndpoint', 'http://127.0.0.1:1337/v1');
            out.model = out.model || vsc.get<string>('janaiModel', '');
            break;
        case 'openai':
            out.endpoint = out.endpoint || vsc.get<string>('openaiEndpoint', 'https://api.openai.com/v1');
            out.model = out.model || vsc.get<string>('openaiModel', 'gpt-4o');
            out.apiKey = out.apiKey || vsc.get<string>('openaiApiKey', '');
            break;
        case 'vscode-lm':
            out.model = out.model || vsc.get<string>('vscodeLmModel', '');
            break;
    }
    return out;
}
